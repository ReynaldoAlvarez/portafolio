...
const Header = () => {
  ...
  return (
    <section className='font-Inter'>
      <div className={`flex fixed items-center justify-between top-0 text-white ${below1050 ? "w-full p-2" : "m-auto w-11/12"} ${isMobile ? "bg-darkgray w-full" : ""}`}>
      ...
      <div className={`middle-nav ease-in duration-300 font-Roboto_mono ${isMobile ? "mt-44 absolute right-0 top-[-100px] bg-darkgray w-full z-40" : (below850 ? "mt-[-500px] " : "")}`} onClick={() => setMobile(false)}>
        ...
        <div className={`end-nav flex space-x-10 items-center  ${below950 ? "space-x-5" : ""}`}>
            ...
            <Link href='https://linkedin.com/in/vidal-reynaldo-alvarez-challco-46633584'>
              <IoLogoLinkedin className={`text-2xl ${twitter ? "text-green" : ""}`}/> 
              </Link>
            ...
            <div className="github flex space-x-2 items-center cursor-pointer" onMouseEnter={handleMouseEnter5} onMouseLeave={handleMouseLeave5}>
              ...
              <AiOutlineGithub className={`text-2xl ${github ? "text-green" : ""}`}/> 
              </Link>
              
            </div>
            <div className='relative flex h-[40px] items-center cursor-pointer justify-center before:absolute before:h-[30px] before:w-[30px] before:rounded-50'>
            <Link href={"https://www.youtube.com/@bunkerdev"}>
              <BsYoutube className={`text-2xl ${github ? "text-green" : ""}`}/> 
              </Link>
            </div>
